...
int                 main(void)
{
    ...
    while (1)
    {
        ...
        // type
        else if (!strcmp(ord_ptr, "type"))
        {
            char* file = strtok(NULL,LIMITER);
            //if it is a error
            if((EOS = strtok(NULL, LIMITER)))
            {
                printf("Too Many Arguments!\n");
                continue;
            }
            if(type(file)) push(ord);
        }
        
        // assemble
        else if (!strcmp(ord_ptr, "assemble"))
        {
            char* file = strtok(NULL, LIMITER);
        ...
        // progaddr
        else if(!strcmp(ord_ptr, "progaddr"))
        {
            char * addr = strtok(NULL,LIMITER);
            if((EOS = strtok(NULL, LIMITER)))
            {
        ...
        //loader
        else if(!strcmp(ord_ptr, "loader"))
        {
            char * file1 = strtok(NULL, LIMITER);
            char * file2 = strtok(NULL, LIMITER);
            char * file3 = strtok(NULL, LIMITER);
            if((EOS = strtok(NULL, LIMITER)))
            {
                printf("Too Many Arguments!\n");
                continue;
            }
            if(loader(file1, file2, file3))
            {
                push(ord);
            }
        }
        // breakpoint
        else if (!strcmp(ord_ptr, "bp"))
        {
            char * opt = strtok(NULL, LIMITER);
            //error occured
            if ((EOS = strtok(NULL, LIMITER)))
            {
                printf("Too Many Arguments!\n");
